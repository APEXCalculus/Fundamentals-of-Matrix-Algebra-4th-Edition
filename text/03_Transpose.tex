...
\index{transpose!definition}\index{matrix!transpose}Let \tta\ be an $m\times n$ matrix. The \textit{transpose of \tta}, denoted $\ttat$, is the $n\times m$ matrix whose columns are the respective rows of \tta.}\index{transpose!definition}
...
There are two answers that each answer both of these questions. First, we are interested in the transpose of a matrix and symmetric matrices because they are interesting.\footnote{At least, interesting for some folks.} One particularly interesting thing about symmetric and skew symmetric matrices is this: consider the sum of $(\tta+\ttat)$ and $(\tta-\ttat)$:
$$(\tta+\ttat)+(\tta-\ttat) = 2\tta.$$
